...
\section{Summary}
The number of discovered exoplanets has dramatically increased in the past decade, allowing scientists to study planetary systems outside the Solar System. 
...
Instead, the WHDS integrator (Section~\ref{sec:whds}) is the optimal choice, since it relies on heliocentric coordinates for integrating bodies and solves the two-body and restricted three-body problems exactly. 
...
In summary, a self-consistent model of MMR planets embedded in a planetesimal disk has yet to be made, which can be accomplished with \hermes.
