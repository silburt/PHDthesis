...
For example, most orbital parameters required for dynamical studies, like eccentricity and mass, is difficult to determine for individual systems via the transit method alone. 
It is only when Transit Timing Variations \citep{Holman2005} or secondary transits are observed that information can be collected about eccentricity and mass via the transit method.
...
The formation of small, rocky protoplanets occurs shortly after the formation of large, kilometre-sized planetesimals.
...
Since numerous \kep planets larger than Earth reside inside the snowline of their host star, this suggests that either i) giant impacts between protoplanets, ii) inward drift of pebble-sized material, and/or iii) planetary migration must have taken place \citep{Schlichting2014}.  
...
Capture probabilities are related to the order of the resonance ($= p - q$), the magnitude of $p$ and $q$, and  the masses, eccentricities and mean motions of the planets involved \citep{SSD1999}.
...
These resonances are driven by the same principles as MMR, and occur when the radial frequency of gas in the disk, $\Omega_g(r)$, matches the planet's radial frequency, $\Omega_p$, according to $m(\Omega_g(r) - \Omega_p) = \pm \kappa(r)$, where $m$ is an integer and $\kappa(r)$ is the epicyclic (or radial) frequency of the gas \citep{SSD1999, Armitage2010}.
...
In contrast to this standard view, recent work \citep{Fung2017} has suggested that planets actually do not migrate that much and tend to be more stationary than originally believed. 
...
\end{equation}
where $\delta_{ij}$ is the Kronecker delta, and the Poisson bracket is defined as $\{f, g\} = \sum_{i=1}^N (\frac{\partial a}{\partial r_i} \frac{\partial g}{\partial p_i} - \frac{\partial a}{\partial p_i} \frac{\partial g}{\partial r_i})$. 
...
This is in contrast to other non-Hamiltonian frameworks, like e.g. the Stormer integrator \citep{Sussman1988} in which the errors of relevant quantities (like position) grow quadratically with time \citep{Saha1992}.
...
In Jacobi co-ordinates the Hamiltonian for a system of n-bodies is \citep{Rein2015b}:
...
In addition, numerical implementation of the N-body problem in this coordinate system calculates the Two Body and Circular Restricted Three Body Problems exactly, meaning that $H_K = H$ and $H_I = 0$ \citep{Hernandez2016}. 
...
The Hamiltonian in Democratic Heliocentric coordinates are \citep{Duncan1998}:
\begin{align}
\begin{split}
...
However, numerical implementation of the N-body problem in this coordinate system does not calculate the Two Body Problem or Circular Restricted Three Body Problems exactly, meaning that non-negligible error terms are retained with $H_K \ne H$ and $H_I \ne 0$ \citep{Hernandez2016}. 
In addition, an additional Jump Hamiltonian $\Ham_J$ is required to keep $\Ham_K$ in the desired format, and the evolution of $\Ham$ in Equation~\ref{eq:DKD} must be modified to include an additional evolution operator under the Jump Hamiltonian, $E_{\Ham_J}$.
