...
If however the planet-planet gravitational interactions are strong, Equation~\ref{eq:RVsum} is invalid and must be replaced with either a more careful numerical treatment or analytic perturbation theory. 
...
In addition, assuming an isotropic distribution of orientations the probability of a transit $P_t$ is inversely proportional to its semi-major axis:
\begin{equation}
P_t = \frac{r_*}{a}
\label{eq:tp}
\end{equation}
...
For example, most orbital parameters required for dynamical studies, like eccentricity and mass, are difficult to determine for individual systems via the transit method alone. 
It is only when Transit Timing Variations \citep{Holman2005} or secondary transits are observed that information can be collected about eccentricity and mass for individual systems.
...
To calculate the detection completeness of known planets \citet{Fressin2013} used the Combined Differential Photometric Precision (CDPP), which represents the effective white noise of a target star for a specific transit duration \citep{Christiansen2012}.
In contrast, \citet{Petigura2013} performed an injection and recovery of simulated light curves to obtain the detection completeness. 
...
\citet{Hayashi1981} and \citet{Weidenschilling1977} provided the first benchmarks for the initial mass of the Solar System disk by assuming a rocky core for all planets and calculating the initial surface density distribution $\Sigma$ as a function of distance $d$ required to produce the masses of the planets:
\begin{equation}
...
Before planets can form, it is believed that an abundance of kilometre-sized planetesimals must be present.
...
The formation of rocky protoplanets occurs shortly after the formation of large, kilometre-sized planetesimals.
...
Since numerous \kep planets larger than Earth reside inside the snowline of their host star, this suggests that either i) giant impacts between close-in protoplanets, ii) inward drift of pebble-sized material, and/or iii) (proto)planetary migration must have taken place \citep{Schlichting2014}.  
...
\label{sec:MMR}
Mean motion resonance (MMR) occurs when the orbital period of one planet is close to an integer ratio of another. 
...
For planetary systems near a $p:q$ MMR (where $p$ and $q$ are integers), two important resonant angles are:
\begin{align}
\begin{split}
...
Other resonant angles exist (e.g. those corresponding to inclination resonances), and linear combinations of resonant angles are possible (e.g. $\phi_3 = \phi_1 - \phi_2  = \varpi_1 - \varpi_2$). 

For planets to be in MMR, at least one resonant argument must librate about a fixed value such that $\dot{\phi}_i = 0$.
...
\begin{equation}
\ddot{\phi} = -A_0^2 \sin\phi
\label{eq:pendulum}
\end{equation}
where $A_0$ is the amplitude of libration and is dependent upon the orbital parameters of the system (mass, eccentricity, semi-major axis).
...
When a pair of planets migrates in an adiabatic fashion, $J = \int Id\phi$ is a conserved quantity of the Hamiltonian system where ($I$, $\phi$) are the action-angle pair \citep{Henrard1982}. 
Adiabatic migration is established when the characteristic time-scale of the orbital migration $\Delta t$ exceeds the libration period, $\tau_{lib}$, of the interacting bodies \citep{Goldreich2014, Batygin2015}.
For planets migrating adiabatically into MMR, if the area enclosed by $J$ is less than the area enclosed by the homoclinic orbit, $J_{homo}$, capture into resonance is guaranteed and otherwise is probabilistic \citep{Henrard1982, SSD1999}.
Capture probabilities are related to the order of the resonance ($= p - q$) and the magnitude of $p$ and $q$, as well as the masses, eccentricities and mean motions of the planets involved \citep{SSD1999}.
...
Statistical excesses of planets exist wide of the 3:2 MMR and 2:1 MMR \citep{Lissauer2011,Fabrycky2014,Steffen2015}, and it is believed that these planets were initially captured in MMR via convergent migration and subsequently transported from MMR via dissipative mechanisms.
...
These resonances are driven by the same principles as MMR, and occur when the orbital frequency of gas in the disk, $\Omega_g(r)$, match the planet's orbital frequency, $\Omega_p$.
The specific requirement for a Linblad resonance is $m(\Omega_g(r) - \Omega_p) = \pm \kappa(r)$, where $m$ is an integer and $\kappa(r)$ is the epicyclic frequency of the gas \citep{SSD1999, Armitage2010}.
...
In contrast to this standard view, recent work \citep{Fung2017} has suggested that planets tend to be more stationary than originally believed. 
...
For example, \citet{Wisdom1980} and \citet{Duncan1989} showed that for small eccentricities in the Restricted 3-Body Problem (R3BP), chaotic orbits (leading to close encounters, collisions and ejections) will occur when the perturber and particle are separated by $\Delta a \le 1.3\mu_p^{2/7}a_p$, where $\mu$ is the planet-star mass ratio and subscript $p$ indicates the perturber. 
...
A 10 billion year integration or the Solar System takes months to complete, and due to the chaotic nature of planetary systems hundreds to thousands of realizations must be simulated to acquire statistically rigorous results. 
...
\end{equation}
where $\delta_{ij}$ is the Kronecker delta, and the Poisson bracket is defined as:
\begin{equation}
\{f, g\} = \sum_{i=1}^N (\frac{\partial f}{\partial r_i} \frac{\partial g}{\partial p_i} - \frac{\partial f}{\partial p_i} \frac{\partial g}{\partial r_i})
\end{equation}
A primary benefit of using a Hamiltonian framework is its symplectic property which enforces the conservation of phase-space $d\textbf{r} \land d\textbf{p}$ \citep{Saha1992}. 
This is in contrast to other non-Hamiltonian frameworks, like e.g. the Stormer integrator \citep{Sussman1988} in which the errors of desired quantities (like position) grow quadratically with time \citep{Saha1992}.
...
\label{sec:coords}
As mentioned above, an efficient way to solve the N-body problem is to split the Hamiltonian into Keplerian and Interaction components.
...
A few popular coordinate systems used in planetary dynamics are discussed below.
...
In Jacobi co-ordinates, the Hamiltonian is split according to \citep{Rein2015b}:

\begin{align}
\begin{split}
\Ham_K &= \sum_{i=1}^{N-1} \frac{\textbf{p}^{\prime,2}_i}{2m^{\prime}_i} - \frac{GM_im^{\prime}_i}{\textbf{r}^{\prime}_i} \\
\Ham_I &= \sum_{i=1}^{N-1} \frac{GM_im^{\prime}_i}{\textbf{r}^{\prime}_i} - \sum_{i=0}^{N-1}\sum_{j=i+1}^{N-1} \frac{Gm_im_j}{|\textbf{r}_i - \textbf{r}_j|}
\end{split}
\end{align}
where $\textbf{r}^{\prime}$ and $\textbf{p}^{\prime}$ are the Jacobi position and momentum, $M_i = \sum_{j=0}^i m_j$,  $m^{\prime}_i = m_iM_{i-1}/M_i$, and $N$ is the number of bodies in the system.
$\textbf{r}^{\prime}$ and $\textbf{p}^{\prime}$ form a canonical set and are related to the cartesian position and momentum (\textbf{r}, \textbf{p}) according to \citep{SSD1999}:
\begin{equation}
...
The benefit of Jacobi coordinates is that the kinetic terms in the Hamiltonian remain a sum of squares \citep{Plummer1918}, making numerical integration a straightforward process according to Equation~\ref{eq:DKD}. 
In addition, numerical implementation of the N-body problem in this coordinate system calculates the Two Body and Circular Restricted Three Body Problems exactly, meaning that $\Ham_K = \Ham$ and $\Ham_I = 0$ \citep{Hernandez2016}. 
...
The Hamiltonian in Democratic Heliocentric coordinates is split according to \citep{Duncan1998}:
\begin{align}
\begin{split}
\Ham_K &= \sum_{i=1}^{N-1} \frac{\textbf{P}^2_i}{2m_i} - \frac{Gm_0m_i}{\textbf{Q}_i} \\
\Ham_I &= - \sum_{i=1}^{N-1}\sum_{j=i+1}^{N} \frac{Gm_im_j}{|\textbf{Q}_i - \textbf{Q}_j|} \\
\Ham_J &= \frac{1}{2m_0} \left|\sum_{i=1}^N \textbf{P}_i\right|^2 
\end{split}
\end{align}

where $m_0$ is the mass of the central body, and $\textbf{Q}$ and $\textbf{P}$ are related to the cartesian position and momenta according to \citep{Duncan1998}:
\begin{equation}
...
However, numerical implementation of the N-body problem in this coordinate system does not calculate the Two Body Problem or Circular Restricted Three Body Problems exactly, meaning that non-negligible error terms are retained with $\Ham_K \ne H$ and $\Ham_{\beta} = \Ham_J + \Ham_I \ne 0$ \citep{Hernandez2016}. 
In addition, a Jump Hamiltonian $\Ham_J$ is required and the evolution of $\Ham$ in Equation~\ref{eq:DKD} must be modified to include an additional evolution operator under the Jump Hamiltonian, $E_{\Ham_J}$.
...
The benefit of these coordinates is that, unlike Democratic Heliocentric coordinates, numerical integration of the N-body problem in this coordinate system calculates the Two Body and Circular Restricted Three Body Problems exactly \citep{Hernandez2016}. 
...
\label{sec:inttypes}
I will explain three classes of N-body integrators: symplectic, non-symplectic and hybrid. 
...
The most popular symplectic integrator for simulating planetary systems is the Wisdom-Holman mapping \citep{Wisdom1991}.
