...
For planets to be in MMR, one or both resonant arguments must librate about a fixed value such that $\dot{\phi}_i = 0$.
...
The magnitude of the Resonant Hamiltonian and libration width are inversely related to the order of the resonance ($= p - q$) and the magnitude of $p$ and $q$ \citep{SSD1999}.
More fundamentally, the magnitude of the Resonant Hamiltonian and libration width are related to the mass, eccentricity and mean motions of the planets involved \citep{SSD1999}.
When a pair of planets migrates near MMR in an adiabatic fashion, $J = \int Id\phi$ is a conserved quantity of the Hamiltonian system \citep{Henrard1982}, where ($I$, $\phi$) are the action-angle pair. 
Adiabatic migration is achieved when the characteristic time-scale of the orbital convergence $\Delta t$ exceeds the libration period $\tau_{lib}$ \citep{Goldreich2014, Batygin2015}.
For planets undergoing convergent and adiabatic migration, if the area enclosed by $J$ is less than the area enclosed by the homoclinic orbit, $J_{crit}$, capture into resonance is guaranteed \citep{Henrard1982, SSD1999}, while otherwise is probabilistic in nature.

Capture probability is strongly coupled to the order of the resonance, with the 3:2 and 2:1 MMRs having the highest capture probabilities \citep{SSD1999, Batygin2015}. 
...
The Jacobi position $\textbf{r}^{\prime}$ and momentum $\textbf{p}^{\prime}$ form a canonical set and are related to the cartesian position and momentum (\textbf{r}, \textbf{p}) according to \citep{SSD1999}:
\begin{equation}
