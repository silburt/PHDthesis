...
\label{sec:RV}
A planet and star orbit around their common centre of mass, and since all stars emits light a periodic wobble can be observed. 
...
When multiple planets orbit a star the radial velocity signal becomes more complex, though it is still quasi-periodic in nature (assuming orbital stability). 
...
For example, most orbital parameters critical for dynamical analysis, like eccentricity and mass, cannot be determined for individual systems via the transit method.
...
However, sufficient detection across the full range of Earth-Jupiter sized planets has enabled scientists to calculate the occurrence of planets around other stars in our galaxy. 
...
Although the injection and recovery technique is certainly a more accurate method to calculate detection completeness, \citet{Fressin2013} found that CDPP estimates were robust and could lead to accurate predictions. 
...
With such large error bars present in the \kep data, ignoring them can affect the resulting occurrence distribution.
...
In standard formation theory the snowline marks the region beyond which water ice forms, providing the additional solid material required to retain gases and form Jovian planets.
...
At present there are two leading models for forming such kilometre-sized planetesimals -- coagulation (core-accretion) and gravitational instability. 
In the coagulation model, pairwise collisions between sticky dust particles lead to steady growth up to a meter in size and beyond \citep{Weidenschilling1977b, Armitage2010}.
However, mean collision velocities are strongly coupled to size, and destructive collisions occur for meter-sized objects \citep{Weidenschilling1977b, Blum2008}, hindering planetesimal growth. 
...
This runaway process is initiated by the fact that $1$--$10$ km sized planetesimals are large enough to gravitationally focus each other yet are still dynamically cool via gas damping, resulting in a maximum collision cross section \citep{Armitage2010}.
...
Since numerous Kepler planets larger than Earth reside inside the snowline of their host star, this suggests that either giant impacts between protoplanets, the inward drift of additional material or migration must have taken place \citep{Schlichting2014}.  
...
Below outlines the two primary ways that planets can migrate -- via planetesimal-driven and/or gas-driven migration.
...
\end{equation}
A primary benefit of using a Hamiltonian framework is the ease of evolving a N-body system into the future or past via Hamilton's equations:
\begin{align}
\begin{split}
...
In the context of planetary systems, Equation~\ref{eq:H} can be modified to make integration more efficient.
For example, we know that a planet will orbit in a Keplerian fashion around a central star, determined exactly by the planet's orbital parameters and stellar mass. 
...
From this a new Hamiltonian can be constructed, consisting of a "Keplerian" term, $\Ham_K$, and "Interaction" term, $\Ham_I$ according to \citep{Wisdom1991}:
\begin{equation*}
\Ham = \Ham_K + \Ham_I
\end{equation*}
...
The benefit of Democratic Heliocentric coordinates is that they do not directly require other bodies to derive their positions (unlike Jacobi coordinates) and are conceptually easy to understand. 
However, these coordinates do not solve the Two Body Problem or Restricted Three Body Problems exactly. 
In addition, $\Ham_K$ cannot be cleanly written as shown in Equation~\ref{eq:Hgeneral} due to several cross terms that arise when substituting in $\textbf{Q}$ and \textbf{P}.
...
Section~\label{Fill} presents \hermes, a hybrid integrator for simulating planetesimal migration, built from \ias \citep{Rein2015a} and \whfast \citep{Rein2015b}. 
...
As mentioned in Section~\ref{sec:migration}, planets near MMR are likely to have formed via migration, and in addition to deriving updated  orbital parameters I also analyze the formation and longterm stability of the system. 
