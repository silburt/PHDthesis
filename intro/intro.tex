...
%\memoas{This paragraph needs to be stronger. Summarize better what we have learned from these disks.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{This Work}
The work presented in this thesis is focused on the statistics, formation and stability of planetary systems. 
The first part of this work deals with a statistical analysis of \kep planets. 
From Section~\ref{sec:stats}, the statistical analysis of \kep planets conducted by \citet{Fressin2013} and \citet{Petigura2013} ignored the large error bars associated with the radii of these planets.
It was unclear how these errors would affect the radius distribution, motivating my work in Section~\label{Fill}. 

In Section~\label{Fill} I investigate the role of tides in bringing planets from exact MMR commensurability to a few percent wide. 
From Figure~\ref{fig:kepMMR}, an excess of planets is seen wide of the 3:2 MMR, and to some degree the 2:1 MMR as well. 
As stated in Section~\ref{sec:MMR}, it is believed that these planets were originally trapped in MMR and some mechanism transported them a few percent wide. 
Other studies have investigated the role of tides in transporting planets from MMR using analytical \citep{Lee2013, Delisle2014} means, but no one had yet to investigate this question from a purely N-body standpoint, thus avoiding any approximations. 

In Section~\label{Fill} I outline \hermes, a hybrid integrator for simulating planetesimal migration, built from \ias and \whfast. 
The original motivation for building this integrator was to investigate the role of planetesimals in transporting planets from MMR. 
Other scientists \citep{Chatterjee2015} have researched this question, but significantly more work still must be done.
...
As stated in Section~\ref{sec:stability}, due to the non-integrable and chaotic nature of multi-planet systems, classification of stability is difficult without performing many costly N-body simulations. 
However, using all the available information from the initial conditions and early evolution, machine learning can accurately predict whether a system will be longterm stable.  
A trained machine learning tool for classifying longterm stability would be invaluable to the scientific community due to the orders of magnitude reduction in classification speed over brute-force N-body integrations.  

In Section~\label{Fill} I analyze the radial velocity data of HD155358 originally collected by \citet{Robertson2012}. 
The system consists of two Jovian-sized planets orbiting a few percent from the 2:1 MMR around a solar-mass star. 
\citet{Robertson2012} did not account for the gravitational planet-planet interactions in deriving orbital parameters, which is a valid assumption when these planets are far from MMR and each other. 
However, since these planets are so close to MMR it motivated a re-analysis of the system. 
