...
\label{eq:H}
\end{equation}
where $\textbf{p}_i$, $\textbf{r}_i$ and $m_i$ are the momentum, distance and mass of body $i$, respectively. 
The first term sums the kinetic energies of the system, while the second term sums the potential energies of the system.

The coordinates ($\textbf{p}$,$\textbf{r}$) used in Hamiltonian mechanics are canonical, obeying the fundamental Poisson bracket relations:
\begin{equation}
\{\textbf{r}_i, \textbf{r}_j\} = 0, \qquad
\{\textbf{p}_i, \textbf{p}_j\} = 0, \qquad
\{\textbf{r}_i, \textbf{p}_j\} = \delta_{ij}
\end{equation}
A primary benefit of encoding an $N$-body system into a Hamiltonian framework lies in its ease of evolving the system into the future or past by applying Hamilton's equations:
\begin{align}
\begin{split}
...
\memoas{Something about symplecticity}

Eq.~\ref{eq:H} can be modified for planetary systems to make the evolution more efficient.
For example, we know that an isolated planet will orbit in a Keplerian fashion around a central star, determined by the planet's orbital parameters and star mass. 
For multi-planet systems gravitational interactions between planets will occur, perturbing them from their ordinary Keplerian motion.
...
As mentioned above the most efficient way to solve the N-body problem for planetary physics is to split the Hamiltonian into Keplerian and Interaction components.
...
\label{eq:Hgeneral}
\end{equation}
However, there are numerous ways to perform these splits, and each coordinate system has different strengths and weaknesses. 
...
\label{sec:Jacobi}
Carl Jacobi worked out a coordinate system in which the planet positions are measured relative to the centre of mass of all bodies interior to it, instead of with respect to the central star (like Democratic Heliocentric Coordinates, see Section~\ref{sec:DH}). 
Since a planet's semi-major axis (and hence position) is dependent upon the mass interior to it (by Kepler's 3rd law), the evolution of a particle is affected by all bodies interior to it, even if they do not directly interact with each other. 

The Jacobi position $\textbf{r}^{\prime}_i$ and momentum $\textbf{p}^{\prime}_i$ of particle $i$ are a canonical set, and are related to the cartesian position and momentum according to \citep{SSD1999}:
\begin{equation}
...
The benefit of Jacobi coordinates is that the kinetic terms remain a sum of squares \citep{Plummer1918}, making numerical integration a straightforward process. 
In addition, these coordinates solve the Two Body Problem and Restricted Three Body Problem exactly. 
...
\label{sec:DH}
In this coordinate system positions $\textbf{Q}$ and momenta $\textbf{P}$ form a canonical set, with $\textbf{Q}$ measured relative to the central mass, and $\textbf{P}$ are barycentric momenta.
...
However, these coordinates do not solve the Two Body Problem exactly nor Restricted Three Body Problem. 
In addition, $\Ham_K$ cannot be cleanly written as shown in Equation~\ref{eq:Hgeneral} due to several cross terms in the expansion.
However, this problem can be rectified by transferring these cross terms into an additional ``Jump" Hamiltonian:
\begin{equation}
\Ham_J = \frac{1}{2m_0} |\sum_{i=1}^N \textbf{P}_i|^2
\end{equation}
Therefore, the evolution of $\Ham$ as shown in Equation~\ref{eq:DKD} must be modified to include an additional evolution operator under the Jump Hamiltonian, $E_{\Ham_J}$.
...
One price to pay for these benefits is that $\{\Ham_J, \Ham_I\} \ne 0$, and in Equation~\ref{eq:DKD}, $E_{\Ham_J}$ must be nestled in between the $E_{\Ham_K}$ and $E_{\Ham_I}$ operators.
